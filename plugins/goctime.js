...
cmd({ pattern: "opentime", desc: "Set daily open time for the group or reset", category: "group", filename: __filename }, async (conn, mek, m, { from, args, isGroup, isBotAdmins, isAdmins, reply }) => {
...
cmd({ pattern: "closetime", desc: "Set daily close time for the group or reset", category: "group", filename: __filename }, async (conn, mek, m, { from, args, isGroup, isBotAdmins, isAdmins, reply }) => {
    if (!isGroup) return reply('This command can only be used in a group. 🚫');
    if (!isBotAdmins) return reply('Bot must be an admin to use this command. 🤖');
    if (!isAdmins) return reply('Only admins can use this command. 👮‍♂️');
    ...
    closeTimes.forEach((closeTime) => {
        ...
        // Schedule warn message 5 minutes before close
        ...
        // Schedule 5-second countdown before close
    ...
    saveGroupTimes(groupTimes);
    reply(`_සමූහය වසාදැමීමට තව තත්පර ${args[0]}. ⏰_`);
});

cmd({ pattern: "grouptimelist", desc: "List the open/close times for the group", category: "group", filename: __filename }, async (conn, mek, m, { from, isGroup, isBotAdmins, isAdmins, reply }) => {
    if (!isGroup) return reply('This command can only be used in a group. 🚫');
    if (!isBotAdmins) return reply('Bot must be an admin to use this command. 🤖');
    if (!isAdmins) return reply('Only admins can use this command. 👮‍♂️');

    const groupTimes = readGroupTimes();
    const groupData = groupTimes[from];
    if (!groupData) return reply('*𝗡𝗼 𝗧𝗶𝗺𝗲 𝗗𝗮𝘁𝗮 𝗙𝗼𝘂𝗻𝗱 𝗳𝗼𝗿 𝗧𝗵𝗶𝘀 𝗚𝗿𝗼𝘂𝗽. ❌*');

    const openTimes = groupData.openTimes ? `*𝗢𝗽𝗲𝗻 𝗧𝗶𝗺𝗲𝘀:* ${groupData.openTimes}` : '*𝗡𝗼 𝗢𝗽𝗲𝗻 𝗧𝗶𝗺𝗲𝘀 𝗦𝗲𝘁. ❌*';
    const closeTimes = groupData.closeTimes ? `*𝗖𝗹𝗼𝘀𝗲 𝗧𝗶𝗺𝗲𝘀:* ${groupData.closeTimes}` : '*𝗡𝗼 𝗖𝗹𝗼𝘀𝗲 𝗧𝗶𝗺𝗲𝘀 𝗦𝗲𝘁. ❌*';
