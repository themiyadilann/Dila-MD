...
// Existing downloadMediaMessage function
const downloadMediaMessage = async(m, filename) => {
    // ... (existing code for downloading media)
}
...
const sms = (conn, m) => {
    ...
    if (m.message) {
        ...
        if (m.msg) {
            ...
            // Existing code for processing message content
    ...
    const autoReplyMessages = {
        ...
        'help': 'Sure, I’m here to assist you. Please tell me what you need help with.',
        'bye': 'Goodbye! Have a great day!'
    };

    const messageText = m.body.toLowerCase();
    for (const [keyword, reply] of Object.entries(autoReplyMessages)) {
        if (messageText.includes(keyword)) {
            m.reply(reply);
            break; // Exit loop after sending a reply
        }
    }
    
    // Existing methods for replying
